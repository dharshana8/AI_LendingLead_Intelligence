...
export default function ProfilePage() {
  const { user, darkMode, addToast } = useApp();
  ...
  const achievements = [
    ...
    { icon: "⭐", title: "AI Champion", desc: "First RM to use AI scoring for 100% leads" },
    { icon: "💰", title: "Revenue Star", desc: "₹2.4Cr business generated in June 2025" },
    { icon: "🎯", title: "Accuracy Award", desc: "94% lead qualification accuracy" },
  ];
      ...
      <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: "16px", alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          ...
          <div style={{ background: cardBg, borderRadius: "12px", border: `1px solid ${border}`, padding: "16px" }}>
            <div style={{ fontSize: "13px", fontWeight: "700", color: textPrimary, marginBottom: "14px" }}>📊 Performance Score</div>
            <div style={{ textAlign: "center", marginBottom: "12px" }}>
              <div style={{ fontSize: "42px", fontWeight: "800", color: "#22c55e" }}>{user?.performance}</div>
              <div style={{ fontSize: "12px", color: textSecondary }}>out of 100</div>
            </div>
            {[["Leads Handled", "20", "#1e40af"], ["Converted", "7", "#22c55e"], ["Avg AI Score", "82", "#f59e0b"], ["Revenue", "₹1.8Cr", "#7c3aed"]].map(([l, v, c]) => (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          ...
          {/* Activity Timeline */}
          <div style={{ background: cardBg, borderRadius: "12px", border: `1px solid ${border}`, padding: "18px" }}>
            <div style={{ fontSize: "14px", fontWeight: "700", color: textPrimary, marginBottom: "14px" }}>📅 Recent Activity</div>
            {[
              { time: "Today 9:15 AM", action: "Viewed Rajesh Kumar's AI profile", type: "view" },
              { time: "Today 9:00 AM", action: "Logged into dashboard", type: "login" },
              { time: "Yesterday 4:30 PM", action: "Initiated outreach to Meera Nair", type: "outreach" },
              { time: "Yesterday 2:00 PM", action: "Generated AI Lead Report", type: "report" },
              { time: "Jun 27, 11:00 AM", action: "Converted Vikram Singh — Home Loan ₹38L", type: "convert" },
            ].map(({ time, action, type }, i) => (
              <div key={i} style={{ display: "flex", gap: "12px", paddingBottom: "12px", marginBottom: "12px", borderBottom: i < 4 ? `1px solid ${border}` : "none" }}>
                <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: type === "convert" ? "#22c55e" : type === "outreach" ? "#1e40af" : "#94a3b8", marginTop: "5px", flexShrink: 0 }} />
                <div>
                  <div style={{ fontSize: "12px", fontWeight: "500", color: textPrimary }}>{action}</div>
                  <div style={{ fontSize: "11px", color: textSecondary, marginTop: "2px" }}>{time}</div>
                </div>
              </div>
            ))}
          </div>
